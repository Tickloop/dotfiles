...
`gd` replaces the native LSP `Ctrl-]` shortcut because Linux uses `Ctrl-]`
for the next tab. `gK` replaces the native insert-mode `Ctrl-S` shortcut
because Linux uses `Ctrl-S` to save. Go uses `gopls`, Python uses
